...
**Memory planning:**
```
Active GPU memory ≈ conversation-threads × window × kv_per_token × layers
Active CPU memory ≈ conversation-threads × kessie-cache-size × 4 bytes (token store)
                   + conversation-threads × (kessie-cache-size/128) × 1KB (index)
```

**Example: 4 threads, 10M cache, 131K window, 32B model:**
- GPU: 4 × 131K KV windows + model weights
- CPU per conversation: ~38MB tokens + ~76MB index = ~114MB
- CPU total: 4 × 114MB = ~456MB active (idle conversations evicted)
